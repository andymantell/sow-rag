...
5. **Assess test document quality.** Read each test PDF from the test folder (default `test-sows/`) — extract via `python SoWImprover/pdf_to_markdown.py <path>` and read the output. For each document, assess:
   - **Realism** — does it read like a genuine SoW or an obvious synthetic/template document? Are there signs of AI generation (generic filler, implausible specifics, boilerplate that doesn't quite fit)?
   - **Completeness** — how many of the 15 canonical sections does it cover? Are any sections stubs or single sentences?
   - **Baseline quality** — how well-written is the original? A document that's already polished gives little room for improvement; a very poor document tests the system's ability to add value. Rate each on a rough scale: poor / adequate / good.
   - **Domain diversity** — across the full test set, how much variety is there in domain, buyer, supplier type, and contract structure? Flag any clustering or gaps.
   - **Suitability as test material** — given the above, how useful is each document for evaluating RAG vs baseline? A test document in a domain far from the corpus tests retrieval failure gracefully; one close to the corpus tests whether retrieval actually helps.
   Summarise findings in a table (one row per document) plus a paragraph on the overall test set composition.

6. Do qualitative analysis — read the actual original, baseline, and RAG content from the qualitative samples in step 2. What changed? Was the RAG version genuinely better, or just different?

7. **Compare with previous report.** Check `reports/` for the most recent prior `experiment-report-*.md`. If one exists:
   - Read it in full
...
8. Write the report following this structure:
   ...
   - **Test Document Assessment** — your independent qualitative review of each test document: realism, completeness (canonical section coverage), baseline writing quality, and suitability as test material. Include a summary table and a paragraph on overall test set diversity and gaps.
...
9. Save to `reports/experiment-report-YYYY-MM-DD.md` (use today's date). If a report with today's date already exists, append an incrementing number: `experiment-report-YYYY-MM-DD-2.md`, `experiment-report-YYYY-MM-DD-3.md`, etc. Never overwrite a previous report.
